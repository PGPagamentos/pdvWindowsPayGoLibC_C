/*********************** SETIS - Automação e Sistemas ************************
 ...
 Descrição        : Console de testes estendido para a simulação de um cliente
                    para a dll de integração com a solução Pay&Go WEB.
 ================================= HISTÓRICO =================================
 Data      Responsável Modificação
 --------  ----------- -------------------------------------------------------
 10/Abr/2019 Erwin      - Alteração da forma de captura de dados em PWDAT_MENU,
                          de forma que possa receber resultado com mais de 1
                          dígito.
 16/Abr/2019 Erwin      - Alteração na forma de captura de dado de cartão digitado,
                          de forma que seja possível se apagar o 1° dígito inserido.
 14/Mai/19 Guilherme    - Remoção de warnings de compilação para 64 bits (CA19-0089).
 20/Mai/19 Guilherme    - Criação da função TesteRemoveCard para simular uma transação 
                          com a remoção do cartão feita pela automação (CA19-0093).
 22/Mai/19 Guilherme    - Alteração na função TesteRemoveCard para fazer a remoção 
                          do cartão em transações negadas e aceitar valores variados
                          (CA19-0093).
 31/Mai/2019 Erwin      - Acrescentado função iGetStringEx para captura de dados
                          com tamanho limite. Alterado o fluxo de venda para captura
                          do valor da transação, código da moeda e expoente da moeda
                          anterior à inserção do cartão.
 24/Jun/2019 Erwin      - Acrescentado tag PWINFO_TRNORIGLOCREF (CA19-0123).
 29/Ago/2019 Erwin      - Acrescentado tratamento quando recebido captura de dado PWDAT_USERAUTH (CA19-0162).
 09/Set/2019 Mateus V.  - Adiciona GetOperationsEx.
 05/Nov/2019 Mateus V.  - Só solicita valor da venda quando a biblioteca já está inicializada. 
 28/Fev/2020 Mateus V.  - Passa a salvar dados da confirmação em arquivo, para eventual queda de energia.
                          Passa a solicitar a confirmação/desfazimento da transação ao final do teste de venda.
 31/Mar/2020 Guilherme  - No caso da execução de menus com somente uma opção, caso a opção default seja ela mesma, 
                          escolhe automaticamente, senão, exibe a opção única para confirmação.
 31/Mar/2020 Erwin      - Criado condição para ser possível se pressionar a tecla ESC quando retornado PWRET_NOTHING.
 06/Mai/2020 Guilherme  - Implementação da confirmação positiva no PIN-pad.
 20/Mai/2020 Mateus V.  - Remoção de warnings para x64.
 15/Jun/2020 Mateus V.  - Correção da seleção de menu para seleções com mais de 20 itens.
 02/Jul/2020 Guilherme  - Inclusão da importação da função pPW_iPPPositiveConfirmation(CA20-128).
 18/Set/2020 Guilherme  - Inclusão do suporte a "exibição" de QR code. (CA20-0148).
 21/Out/2020 Guilherme  - Passa a checar a necessidade de envio de desfazimento para transações 
                          canceladas pelo usuário (CA20-0159).
\*****************************************************************************/
#include "stdio.h"
#include "conio.h"
#include "windows.h"
...
#define  PGWEBLIBTEST_VERSION "1.5.0.5"
...
// Capacidades da automação tratadas por todas transações ao aplicativo de testes
...
/*=====================================================================================*\
 ...
 Descricao  :  Responsável por enviar a confirmação.
 
 Entradas   :  ulCause: Código da causa da confirmação.
...
static const char* pszGetInfoDescription(Int16 wIdentificador)
{
   switch(wIdentificador)
   {
      ...
      case(PWINFO_UNIQUEID         ): return "PWINFO_UNIQUEID"; 
...
/*=====================================================================================*\
 ...
 Descricao  :  Responsável por salvar os dados da transação para enviar a confirmação em caso
               de queda de energia.
...
static void SalvaDadosTransacao(void) 
{
   ...
   if (iRet == PWRET_OK) {
      ...
      strcpy_s( gstConfirmData.szHostRef, sizeof(gstConfirmData.szHostRef), "");
   ...
   if (iRet == PWRET_OK) {
      ...
      strcpy_s( gstConfirmData.szLocRef, sizeof(gstConfirmData.szLocRef), "");
   ...
   if (iRet ==PWRET_OK) {
      ...
      strcpy_s( gstConfirmData.szVirtMerch, sizeof(gstConfirmData.szVirtMerch), "");
   ...
   if (iRet == PWRET_OK) {
      ...
      strcpy_s( gstConfirmData.szAuthSyst, sizeof(gstConfirmData.szAuthSyst), "");

   if (iCont == 0)
      return;

   fopen_s(&fpConfirm, gszConfirmFileName, "w");
...
static Int16 iExecGetData(PW_GetData vstGetData[], Int16 iNumParam)
{
   ...
   char szAux[50001], szDspMsg[128], szMsgPinPad[34];
      ...
      switch( vstGetData[i].bTipoDeDado)
      {
         ...
         case(PWDAT_MENU):
            ...
            else
            {
               ...
               printf("\n\nSELECIONE A OPCAO:");
               iAux = 0;
               sprintf_s (szAux, sizeof(szAux), "%d", vstGetData[i].bNumOpcoesMenu);
               for (k = 0; k < (Int16) strlen (szAux); k++) {
                  do 
                  {
                     iKey = _getch ();
...
#define fGetString_(a,b) fGetString(a,sizeof(b),GETS_NOSZ|GETS_FIXEDLEN|GETS_NODEFAULT,b)
#define fGetStringDef_(a,b) fGetString(a,sizeof(b),GETS_NOSZ|GETS_FIXEDLEN,b)
...
static int iGetStringEx (const char *pszMsg, int iMaxChar, Word wFlags, char *pszData)
{
   ...
   if (wFlags & GETS_NOSZ) {
      ...
      if (--iMaxChar < 0) return 0;
      if (wFlags & GETS_NODEFAULT)
         iIdx = 0;
      else 
         iIdx = (int) strlen (pszData);
      if (iIdx > iMaxChar) return -1;
   }
   ...
   for (i = 0; i < iIdx; i++, iOffset++) printf ("%c", pszData[i]);
   ...
   for (;;) {
      ...
      if (iKey >= ' ' && iIdx < iMaxChar) {
         printf ("%c", iKey);
         if (iKey == '#' && (wFlags & GETS_ASCCR)) iKey = '\r';
      ...
      if (iKey == 13 || iKey == 10) {
         ...
         if (!(wFlags & GETS_EVEN) || !(iIdx % 2)) {
            ...
            if (!(wFlags & GETS_FIXEDLEN)) break;

            // Se atingiu o tamanho máximo sempre acata...
            if (iIdx >= iMaxChar) break;
              
            // Se campo estiver vazio e isto é permitido, acata...
            if (!iIdx && (wFlags & GETS_ACCEPTBLK)) break;
         }
      }
...
static void AddMandatoryParams(Uint32 ulAutCap)
{
   ...
   sprintf_s(szAutCap, sizeof(szAutCap), "%lu", ulAutCap);
...
static void Init()
{
   ...
   if (!iRet) {
      ...
      fopen_s(&fpConfirmation, gszConfirmFileName, "r");
...
/*=====================================================================================*\
 ...
 Descricao  :  Esta função executa PW_iPPTestKey.
...
static void PPTestKey()
{
   ...
   // Executa a operação
   iRet = gstHwFuncs.pPW_iPPTestKey(uiIndex);
   PrintReturnDescription(iRet, szDspMsg);
   if(iRet)
      return;
...
static void TesteVenda()
{
   ...
   /* Verifica se tem necessidade de confirmação */
   ...
   for (;;) {
      cKey = _getch ();
      if (cKey == '0') {
         iConfirmaTransacao(PWCNF_REV_MANU_AUT);
         return;
      }
      else if (cKey == '1') {
         iConfirmaTransacao(PWCNF_CNF_AUTO);
...
/*=====================================================================================*\
 ...
 Descricao  :  Esta função aguarda o uso de um catão como "gatilho" para início da 
...
static void TesteAutoatendimento()
{
   ...
   // Adiciona os parâmetros obrigatórios (indicando que não deve ser feita a remoção do cartão do PIN-pad)
   AddMandatoryParams(PGWLT_AUTCAP);
...
int main(int argc, char* argv[])
{
   ...
   else
   {
      ...
      (FARPROC)gstHwFuncs.pPW_iGetOperationsEx     = GetProcAddress  (ghHwLib, "PW_iGetOperationsEx");
      ...
      (FARPROC)gstHwFuncs.pPW_iPPTestKey           = GetProcAddress  (ghHwLib, "PW_iPPTestKey");
      (FARPROC)gstHwFuncs.pPW_iPPPositiveConfirmation = GetProcAddress  (ghHwLib, "PW_iPPPositiveConfirmation");
